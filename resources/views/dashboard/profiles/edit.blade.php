        ...
        <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
            ...
            <form action="{{ route('dashboard.profiles.update', $profile->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ phone: '{{ old('phone', $profile->phone) }}' }">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1">Profile Password (Optional)</label>
                        <input type="text" name="profile_password" value="{{ old('profile_password', $profile->profile_password) }}" placeholder="Leave blank to disable" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-sky-500 text-sm outline-none"/>
                    </div>
                    <div class="flex items-center mt-6">
                        <input type="checkbox" name="enable_lead_capture" id="enable_lead_capture" value="1" {{ old('enable_lead_capture', $profile->enable_lead_capture) ? 'checked' : '' }} class="w-4 h-4 text-sky-600 border-gray-300 rounded focus:ring-sky-500">
                        <label for="enable_lead_capture" class="ml-2 block text-sm font-bold text-slate-700">
                            Enable Lead Capture (Contact Form)
                        </label>
                    </div>
                </div>
