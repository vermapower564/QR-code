    ...
    <div class="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
        ...
        <form action="{{ route('dashboard.settings.profile') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" maxlength="10" pattern="[0-9]{10}" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 10)" placeholder="9876543210" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-sky-500 text-sm outline-none"/>
            </div>
